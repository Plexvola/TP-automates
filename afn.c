...
void afn_init(afn *A, uint nbetat, char *alphabet, ullong init, ullong finals)
{
	...
	for (i = 0; i < A->nbsymb; i++) {
		if ((alphabet[i] < SYMB_ASCII_DEB) || (alphabet[i] > SYMB_ASCII_FIN)) {
			fprintf(stderr, "[afn_init] caractere ascii numero %d invalide\n",
			        alphabet[i]);
			exit(-1);
		}
...
void afn_determinisation(afn A, afd *D)
{
	int i;
	uint *states;
	states = calloc(INT_ETAT(A.nbetat) - 1,
	                sizeof(uint)); // dictionnaire états d'afn -> nom afd
	file taint = NULL;
	ullong p, q, accessed;
	...
	while (taint != NULL) { // états non-marqués
		...
		for (i = 0; i < A.nbsymb; ++i) { // symboles
			...
			for (int j = 0; p >> j > 0; j++) { // éléments de l'ullong
				if (IN(j, p)) {
					accessed |=
						A.delta[j][A.tsymb[A.alphabet[i] - SYMB_ASCII_DEB]];
				}
			}
...
/*
 * Calcule un automate qui reconnait la concaténation de <A> et <B>
 *    +-+     +-+
 * ->O| |->O->| |->0
 *    +-+     +-+
 *     A       B
